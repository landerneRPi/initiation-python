...
class MenuBar(Frame):
    ...
    def __init__(self, boss =None):
        ...
        # CC => 204 vert
        # #6699ff du bleu
        facileMenu = Menubutton(self, text="Facilité", bd=5, bg="#00CC00",
            activebackground="#6699ff")
        facileMenu.pack(side=LEFT, padx=10)
        mederoul = Menu(facileMenu)
        mederoul.add_command(label='Affichage', underline=0,
                        command=boss.affiche)
        facileMenu.configure(menu=mederoul)
...
class Ping(Frame):
    ...
    def affiche(self):
        "Fenêtre d'affichage"
        # self , c'est la fenêtre prncipale
        msg = Toplevel(self)
        monMessage = Message(msg, width=300, aspect=50, justify=LEFT,
        text="Atelier Raspberry Pi\n à Landerneau\n")
        monMessage.pack(padx=5, pady=5)
